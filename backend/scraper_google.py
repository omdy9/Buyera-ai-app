"""
scraper_google.py  — v9  (Robust imports + graceful LLM degradation)
=====================================================================
Key fixes from v8:
  - All internal imports use try/except for relative vs absolute
  - enrich_company never returns None for valid leads (only quality-gate rejects)
  - Scoring works even when ai_summary is empty
  - parallel_enrich always includes leads when LLM unavailable (quality_threshold=0)
"""
...
BAD_DOMAINS = {
    ...
    "goodfirms","sortlist","upcity","bark","bark.com",
...
def enrich_company(company: dict, query: str = "", quality_threshold: int = 0) -> dict | None:
    ...
    # Step 2: LLM pipeline (fixed import)
    try:
        try:
        ...
        llm = full_llm_pipeline(
            query=query, company=name, website=website, content=crawl["content"])
        ...
        logger.warning("LLM pipeline error for %s: %s — using crawl data only", name, exc)
        # Ensure required fields exist even without LLM
        company.setdefault("ai_summary", company.get("snippet", ""))
        company.setdefault("products", [])
        company.setdefault("is_valid_lead", True)
        company.setdefault("rejection_reason", "")
        company.setdefault("grok_score", 5)
        company.setdefault("is_directory", False)
        company.setdefault("certifications", crawl.get("certifications", []))
        company.setdefault("export_markets", crawl.get("export_markets", []))

    # Quality gate — only reject if above threshold AND no contact
    if quality_threshold >= 2:
        ...
        if not has_contact and not company.get("is_directory"):
            logger.info("Filtered (no contact, threshold=%d): %s", quality_threshold, name)
            return None

    if quality_threshold >= 3:
        if not company.get("is_valid_lead") or int(company.get("grok_score", 0) or 0) < 6:
            logger.info("Filtered (Grok rejected): %s score=%s", name, company.get("grok_score"))
            return None

    # Directory extraction
    if company.get("is_directory") and crawl.get("content"):
        try:
            try:
...
def parallel_enrich(companies: list, query: str = "",
                    quality_threshold: int = 0, max_workers: int = 3) -> list:
    enriched = []
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = {
            ex.submit(enrich_company, c, query, quality_threshold): c
            for c in companies
        }
        for future in as_completed(futures, timeout=150):
            try:
                ...
                # Always keep the lead with basic data rather than dropping it
                c = futures[future]
                c.setdefault("ai_summary",   c.get("snippet", ""))
                c.setdefault("products",     [])
                c.setdefault("is_valid_lead", True)
                c.setdefault("grok_score",   5)
                if quality_threshold < 2:
                    enriched.append(c)
    return enriched
...
def _score(query: str, company: dict) -> dict:
    try:
        ...
        # Ultimate fallback if nlp also fails
        company.setdefault("final_score", 0.3)
        company.setdefault("importance", "low")
        company.setdefault("ai_summary", company.get("snippet", ""))
        return company
    ...
    if not company.get("ai_summary") and combined.strip():
        try:
            ...
            company["ai_summary"] = company.get("snippet", "")
...
def google_search(
    ...
    if not SERP_API_KEY:
        return {
            "companies": [], "next_start": start, "has_more": False,
    ...
    # Fixed import — works both as package and standalone
    try:
        try:
        ...
        logger.error("Cannot import search_strategy: %s — using single query", exc)
        generate_search_queries = lambda q, **kw: [q]

    search_queries = generate_search_queries(query, country=country_filter, city="", industry="")
    ...
    # Mix page 1 + page 2 results for diversity
    query_jobs = []
    for i, sq in enumerate(search_queries):
        query_jobs.append((sq, 0))
        if i < 3:
            query_jobs.append((sq, 10))
    ...
    for r in all_raw:
        link  = r.get("link", "")
        title = r.get("title", "")
        if not link or not _is_valid_company_url(link):
            continue
...
def linkedin_discovery(query: str, country_filter: str | None = None,
    ...
    try:
        ...
        return [
            {"name": r.get("title", ""), "profile": r.get("link", ""), "snippet": r.get("snippet", "")}
